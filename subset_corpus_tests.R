...
# select articles only from 2019 - Juni 2020
subset_textdata <- textdata %>% filter(textdata$date > "2019-12-01" & textdata$date < "2020-07-01" )
# alternativ: subset_textdata <- textdata[which(textdata$date > "2019-12-01" & textdata$date < "2020-07-01")]

# Create corpus
subset_corpus <- corpus(subset_textdata$text, docnames = subset_textdata$id,
                          docvars = data.frame(year = substr(subset_textdata$date,0,4)))
# original corpus length
ndoc(subset_corpus)

substr(texts(subset_corpus)[1],0,200)

# separate sentences
subset_corpus_sentences <- corpus_reshape(subset_corpus, to = "sentences")

ndoc(subset_corpus_sentences)
texts(subset_corpus_sentences)[1]
texts(subset_corpus_sentences)[2]
...
subset_corpus_tokens <- subset_corpus_sentences %>%
...
subset_sotu_collocations <- quanteda.textstats::textstat_collocations(subset_corpus_tokens,
                                                               min_count = 100)
head(subset_sotu_collocations,200)

subset_sotu_collocations <- subset_sotu_collocations[1:150, ]
subset_corpus_tokens <- tokens_compound(subset_corpus_tokens, subset_sotu_collocations)
