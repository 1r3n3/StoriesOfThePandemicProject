...
topicModel3 <- LDA(convert(DTM, to = "topicmodels"), K)
...
for (i in 1:nDocs(DTM)){
  ...
  primarytopic <- order(topicsperdoc, decreasing = TRUE)
...
numb_terms <- 7 # Number of terms to be displayed per topic
...
topic_number <- c(15,17,19,21,23,25)
numb_tokens <- 10
coh_score <- numeric(length(topic_number))
for (i in 1:length(topic_number)){ 
  
  # compute the LDA model, inference via n iterations of
  # Gibbs sampling
  topicModel2 <- LDA(DTM, topic_number[i], method = "Gibbs", control = list(iter = 500,
                                                             seed = 1, verbose = 25))
  
  #Coherence Score
  coh_score[i] <- mean(topic_coherence(topicModel2, DTM, top_n_tokens = numb_tokens))

}

coherence_mat <- data.frame(k = topic_number, coherence = coh_score)

ggplot(coherence_mat, aes(x = k, y = coherence)) +
  geom_point() +
  geom_line(group = 1)+
  ggtitle("Best Topic by Coherence Score") + theme_minimal() +
  scale_x_continuous(breaks = seq(1,25,1)) + ylab("Coherence")




###########################################################################
# Plotten der Ergebnisse des Topic Modellings in Kombination mit Cooc_Analysis
...
numb_terms <- 10
c <- matrix(numeric(K*numb_terms),nrow=numb_terms)
for (i in 1:K){
  topterms <- sort(tmresult$terms[i,], decreasing = TRUE)[1:numb_terms]
  c[1:numb_terms,i] <- names(topterms)
}

# Erstellen der Spaltennamen des Dataframes
b <- numeric(K)
for (i in 1:K){
  b[i] <- paste0("Topic", i)
}
...
# Erstellung der binären DTM notwendig!
# separate sentences
corpus_sentences <- corpus_reshape(guardian_corpus, to = "sentences")

# Preprocessing of the corpus of sentences
corpus_tokens2 <- corpus_sentences %>% # corpus_sentences
  tokens(remove_punct = TRUE, remove_numbers = TRUE, remove_symbols = TRUE) %>% # remove_separators = TRUE
  tokens_tolower() %>%
  tokens_replace(lemma_data$inflected_form, lemma_data$lemma,
                 valuetype = "fixed") %>%
...
coocs <- logsig[-match(coocTerm, names(sig))]
